...
for iel = 1:NT
    ...
    D(end-1,:) = hK/areaK*integralTri(divmm2,4,nodeT,elemT);
    D(end,:) = hK/areaK*integralTri(divmm3,4,nodeT,elemT);
    ...
    I1 = zeros(Nmm, NdofA);
    I1(1:Nm, end-1) = pde.nu*areaK*Lapm;
    I1(Nm+1:end, end) = pde.nu*areaK*Lapm;
    % second term
    I2 = zeros(Nmm, NdofA);   
    elem1 = [v1(:), v2(:), v1(:)+Nv]; % elem2dof for [ae, be, me]
    Gradmm = cell(2,Nmm);
    Gradmm(1,1:Nm) = Gradmc; 
    Gradmm(2,Nm+1:end) = Gradmc;
    for im = 1:Nm
    ...
    for s = 1:2
        ...
        F3 = 4/6*(mc(xe,ye).*Ne);
        F = [F1; F2; F3];
        P0K(s, 1:2*NdofBd) = hK/areaK*accumarray([elem1(:); elem1(:)+NdofBd], F(:), [2*NdofBd 1]);
    end
    % Bs, G, Gs
    Bs = B;  Bs([1,Nm+1], :) = P0K;
    G = B*D;  Gs = Bs*D;
    ...
    Pf = integralTri(fxy,5,nodeT,elemT); 
