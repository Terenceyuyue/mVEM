function [u,info] = elasticityVEM_NCUniformReducedIntegration(node0,elem0,pde,bdNeumann,refineType)
%This function solves linear elasticity equation of tensor form using 
% nonconforming virtual element method in V1 with reduced integration applied.
%
%       u = [u1, u2]
...
%% Compute and assemble L2 rotK and L2 divK projection 
elemLen0 = cellfun('length',elem0); 
nnz = sum((2*(2*elemLen0)).^2); % e -- e1+e2
ii0 = zeros(nnz,1);    jj0 = zeros(nnz,1); 
ssRot = zeros(nnz,1);  ssDiv = zeros(nnz,1);
ia = 0;  s = 0;
for iK = 1:NT0
    ...
    % loop of the macro-elements of K
    Crotx = zeros(Ns,Nc);  Croty = zeros(Ns,Nc);  
    Cdivx = zeros(Ns,Nc);  Cdivy = zeros(Ns,Nc);
    indexC = zeros(Ns,Nc);
    for i = 1:Ns
        iel = s+i;
        ...
        Croty(i,:) = Te(1:Nc,2); 
        Cdivx(i,:) = Ne(1:Nc,1);  % The first two edges of T are the bisected edges
        Cdivy(i,:) = Ne(1:Nc,2); 
        indexC(i,:) = indexEdge(1:Nc);
    end
    s = s+Ns;
    
    % L2 rot projection
    H0K = aux0.area(iK);
    C0rotx = reshape(Crotx',1,[]);   C0divx = reshape(Cdivx',1,[]); 
    C0roty = reshape(Croty',1,[]);   C0divy = reshape(Cdivy',1,[]);
    C0rotK = [C0rotx, C0roty];       C0divK = [C0divx, C0divy];  
    PirotK = H0K\C0rotK;             PidivK = H0K\C0divK;
    BrotK = PirotK'*H0K*PirotK;      BdivK = PidivK'*H0K*PidivK;
    
    % local to global index
    Ndof = Nc*Ns; Ndof2 = 2*Ndof; 
    indexDofx = reshape(indexC',1,[]);
    ...
    ssRot(ia+1:ia+Ndof2^2) = reshape(BrotK',[],1);
    ssDiv(ia+1:ia+Ndof2^2) = reshape(BdivK',[],1);
    ia = ia + Ndof2^2;
end
ssRot = -para.mu*ssRot; 
ssDiv = para.lambda*ssDiv; 
...
kk = sparse([ii;ii0;ii0],[jj;jj0;jj0],[ss;ssRot;ssDiv],2*NE,2*NE);
