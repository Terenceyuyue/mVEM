function [uh,info] = IPVEMH2(node,elem,pde,bdStruct)
...
for iel = 1:NT
    ...
    % H2
    Basexx{iel} = (mxx(xq,yq)*Pis)';
    Basexy{iel} = (mxy(xq,yq)*Pis)';
    Baseyy{iel} = (myy(xq,yq)*Pis)';
end
...
for s = 1:NEb
    ...
    xK = centroid(iel,1); yK = centroid(iel,2); hK = diameter(iel);      
    Dofe = [index,indexEdge+N,iel+N+NE];
    ...
    zq = [xq, yq];
    phix = (mx(xq,yq)*Pi1s)';  % (ndofe,ng), ndofe = 3
    phiy = (my(xq,yq)*Pi1s)'; 
    phixx = (mxx(xq,yq)*Pi2s)'; 
    phixy = (mxy(xq,yq)*Pi2s)'; 
    phiyy = (myy(xq,yq)*Pi2s)'; 
