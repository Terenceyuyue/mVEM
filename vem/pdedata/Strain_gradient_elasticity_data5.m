function pde = Strain_gradient_elasticity_data5(para)
...
% derivative of the exact solution
    function val =  Du(p)  % w = [u1,u2]
        x = p(:,1); y = p(:,2);
        val(:,1) = u1x(x,y) + 0*x;  val(:,2) = u1y(x,y) + 0*x; 
        val(:,3) = u2x(x,y) + 0*x;  val(:,4) = u2y(x,y) + 0*x;
    end
% second order derivative of the exact solution
    function val =  DDu(p)  % w = [u1,u2]
        x = p(:,1); y = p(:,2);
        DDu1(:,1) = u1xx(x,y) + 0*x;  DDu1(:,2) = u1xy(x,y) + 0*x;  DDu1(:,3) = u1xy(x,y) + 0*x; DDu1(:,4) = u1yy(x,y) + 0*x; 
        DDu2(:,1) = u2xx(x,y) + 0*x;  DDu2(:,2) = u2xy(x,y) + 0*x;  DDu2(:,3) = u2xy(x,y) + 0*x; DDu2(:,4) = u2yy(x,y) + 0*x; 
        val = [DDu1, DDu2];
    end
...
function [u1,u2,f1,f2,u1x,u1y,u2x,u2y,u1xx,u1xy,u1yy,u2xx,u2xy,u2yy] = compute_rhs(para)    
    ...
    % exact solution
    u1 = x.*y.*(1-x).*(1-y).*exp(-1000*((x-0.5).^2+(y-0.5).^2));
    u2 = u1;
