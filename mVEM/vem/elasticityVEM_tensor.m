function u = elasticityVEM_tensor(node,elem,pde,bdStruct)
...
% ----------------- D,Bs,G,Gs,H0,C0 ---------------------------
[D,Bs,G,Gs,H0,C0] = VEM_MAT_elasticity_tensor(aux);
...
for iel = 1:NT
    ...
    Pi0s = H0{iel}\C0{iel};
    % Stiffness matrix
    AK  = Pis'*G{iel}*Pis + (I-Pi)'*(I-Pi); AK = 2*pde.mu*AK;
    BK = pde.lambda*Pi0s'*H0{iel}*Pi0s;
