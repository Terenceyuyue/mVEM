...
tic;
% ------------- Mesh --------------------
% generated by meshfun.m in tool folder
load Poissonmesh.mat; % rectangle
%load PoissonmeshCircle.mat;  % circle
% figure, showmesh(node,elem);
% findnode(node); % findelem(node,elem);

% ---------------- PDE data -----------------------
pde = Poissondata();
bdNeumann = 'abs(x-0)<1e-4 | abs(x-1)<1e-4'; % string for Neumann (rectangle)
%bdNeumann = [];
