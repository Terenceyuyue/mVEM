...
%% Initial mesh
load meshLshape1
%load dualmeshLshape1
%load meshdataLshape1 % ???

%% PDE data
pde = biharmonicdata_lowregularity_avem();
pde.a = 1;
theta = 0.4; 
...
for k = 1:maxIt
    ...
    % Step 2: ESTIMATE
    [eta,etaStab] = IPVEM_indicator(node,elem,uh,info,pde,bdStruct);
    etaN(k) = norm(eta);
    ...
    if (size(node,1)>maxN) || (k==maxIt) || h<1e-5
