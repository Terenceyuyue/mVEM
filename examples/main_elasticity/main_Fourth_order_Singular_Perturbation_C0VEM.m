...
% exact
tri = eye(2);
ux = @(x,y) pde.Du([x,y]).^2*tri(:,1);
uy = @(x,y) pde.Du([x,y]).^2*tri(:,2);
Du = @(x,y) ux(x,y)+uy(x,y);
uH1square = squareint(Du,[0,1,0,1]); % square

tri = eye(4);
uxx = @(x,y) pde.DDu([x,y]).^2*tri(:,1);
uxy = @(x,y) pde.DDu([x,y]).^2*tri(:,2);
uyx = @(x,y) pde.DDu([x,y]).^2*tri(:,3);
uyy = @(x,y) pde.DDu([x,y]).^2*tri(:,4);
DDu = @(x,y) uxx(x,y)+uxy(x,y)+uyx(x,y)+uyy(x,y);
