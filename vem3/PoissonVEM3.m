...
centroid3 = aux.centroid3;
diameter3 = aux.diameter3; volume = aux.volume;
% auxstructure3
...
for iel = 1:NT
    ...
    % ------- scaled monomials ----------
    m = @(x,y,z) [1+0*x,(x-xK)/hK,(y-yK)/hK,(z-zK)/hK]; % m1,m2,m3,m4
    ...
    for s = 1:size(elemf,1)
        ...
        % add grad(m)*nf
        Bf = dot(gradmMat, repmat(nf,Nm,1),2)*intProj;
        B = B + Bf;
    end
    % constraint
    Bs = B;  Bs(1,:) = 1/Ndof;
    ...
    fun = @(x,y,z) repmat(pde.f([x,y,z]),1,Nm).*m(x,y,z);
