...
for iel = 1:NT
    ...
    for s = 1:size(elemf,1)
        ...
        Pifs = Pifs(:,idx);
        % outer normal vector
        % Note: The face vertices has been arranged in counterclockwise order           
        ...
        intProj = zeros(1,Ndof3);  % global adjustment
...
if ~isempty(bdFaceN)
    ...
    for s = 1:size(bdFaceN,1)
        % vertices of face
        index = bdFaceN{s};   nv = length(index);
        P = node3(index,:);
        % elliptic projection on the face
        idFace = bdFaceIdxN(s);   
        Pifs = faceProj{idFace}; % the order may be not correct
