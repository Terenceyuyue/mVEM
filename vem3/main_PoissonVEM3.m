...
for k = 1:maxIt
    ...
    fprintf('Mesh %d: \n', k);
    switch meshType 
        case 1    % structured polyhedral mesh
            load( ['SimpleMesh3data', num2str(k), '.mat'] ); 
        case 2    % CVT polyhedral mesh
            load( ['mesh3data', num2str(k), '.mat'] ); 
        case 3    % tetrahedral mesh
            [node3,~,elem3] = cubemesh([0 1 0 1 0 1], 1/(2*k)); 
    end
    % get boundary information
    bdStruct = setboundary3(node3,elem3,bdNeumann);
    % solve
    [uh,info] = PoissonVEM3_NEW(node3,elem3,pde,bdStruct);
    % record
