function pde = Strain_gradient_elasticity_data1(para)
...
% derivative of the exact solution
    function val =  Du(p)  % w = [u1,u2]
        x = p(:,1); y = p(:,2);
        Du1(:,1) = u1x(x,y);  Du1(:,2) = u1y(x,y); 
        Du2(:,1) = u2x(x,y);  Du2(:,2) = u2y(x,y);
        val = [Du1, Du2];
    end
...
function [u1,u2,f1,f2,u1x,u1y,u2x,u2y,u1xx,u1xy,u1yy,u2xx,u2xy,u2yy] = compute_rhs(para)    
    ...
    % exact solution
    u1 = (exp(cos(2*pi*x))-exp(1)).*(exp(cos(2*pi*y))-exp(1));
    u2 = (cos(2*pi*x)-1).*(cos(4*pi*y)-1);    
