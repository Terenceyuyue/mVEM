...
for iel = 1:NT
    % element information
    index = elem{iel};     Nv = length(index);     Ndof = 2*Nv;
...
for iel = 1:NT
    ...
    index = elem{iel};     Nv = length(index);  Ndof = 2*Nv;
    sgnelem = sign(diff(index([1:Nv,1])));
    id = elem2edge{iel}; sgnbd = E(id); sgnelem(sgnbd) = 1;
    sgnbase = ones(Ndof,1); sgnbase(Nv+1:end) = sgnelem;
...
%     fK = pde.f(cK)*areaK*P0;
    ...
    m = @(x,y) pde.f([x,y]).*m(x,y); % f(p) = f([x,y])
    rhs = integralTri(m,2,nodeT,elemT); rhs = rhs';
    fK = Pis'*rhs;
    fK = fK.*sgnF;
