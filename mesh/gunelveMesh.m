% Generate the gunelve mesh used in the Cook's membrane problem, a typical
% benchmark for the elasiticity problems
%
...
square = [0 1 0 1];  Nx = 7;  Ny = Nx;
[node,elem] = gunelveMesh_square([0 1 0 1],Nx,Ny);
figure,
showmesh(node,elem)
title('Gunelve mesh on the square');
filename = ['meshdata_gunelveSquare', num2str(size(elem,1))]; 
save(filename, 'node', 'elem')
...
x = gridSquare(:,1);  
% transform the Gunelve mesh on the unit square into the mesh on the membrane
y = linspace(0,1,Ny+1);
...
%save meshdata_gunelve0 node elem
% 
%% Gunelve mesh on the square
